...
def plot_rollout_grid(input_pgs, pred_pgs, gt_pgs, rollout_length, sample_idx, filename, title=None):
    import matplotlib.pyplot as plt
    fig, axes = plt.subplots(3, rollout_length, figsize=(4*rollout_length, 12))
    row_titles = ['Input', 'Prediction', 'Ground Truth']
    for col in range(rollout_length):
        for row, pg_list in enumerate([input_pgs, pred_pgs, gt_pgs]):
            ax = axes[row, col] if rollout_length > 1 else axes[row]
            if pg_list[col] is not None:
                plot_particle_groups(pg_list[col], ax=ax, show=False)
            if col == 0:
                ax.set_ylabel(row_titles[row], fontsize=16)
            if row == 0:
                ax.set_title(f"Step {col}", fontsize=16)
    if title:
...
def rollout_evaluation(sequence, rollout_length, model, device, results_folder,
    ...
    for t in range(rollout_length):
        ...
        input_pgs.append(input_pg)
        pred_pgs.append(pred_pg)
        gt_pgs.append(gt_pg)
        # Use the new threeway plot function (keep for single-step plots)
    ...
    # Plot the grid for all rollout steps
    grid_filename = os.path.join(results_folder, f"sample_{sample_idx}_rollout_grid.png")
    plot_rollout_grid(input_pgs, pred_pgs, gt_pgs, rollout_length, sample_idx, grid_filename, title=f"Sample {sample_idx} Rollout Grid")
