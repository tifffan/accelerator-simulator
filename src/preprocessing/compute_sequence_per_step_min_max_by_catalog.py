...
def compute_global_min_max_per_step(data_catalog, output_file='sequence_global_min_max.txt', identical_settings=False, settings_file=None):
    # Load data catalog
    ...
    for i, row in df.iterrows():
        ...
        if sum_channels is None:
            ...
            min_channels = torch.full((num_time_steps, num_features), float('inf'), dtype=torch.float64)
            max_channels = torch.full((num_time_steps, num_features), float('-inf'), dtype=torch.float64)
        ...
        for t in range(num_time_steps):
            ...
            min_channels[t] = torch.min(min_channels[t], data_t.min(dim=0).values)
            max_channels[t] = torch.max(max_channels[t], data_t.max(dim=0).values)
    ...
    with open(output_file, 'w') as f:
        ...
        f.write("Per-Step Global Min:\n")
        for t in range(num_time_steps):
            min_str = ", ".join([f"{m.item():.12e}" for m in global_min[t]])
            f.write(f"Step {t}: {min_str}\n")
        f.write("Per-Step Global Max:\n")
        for t in range(num_time_steps):
            max_str = ", ".join([f"{m.item():.12e}" for m in global_max[t]])
            f.write(f"Step {t}: {max_str}\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compute global min, max, and mean per time step from time series particle data.")
    ...
    compute_global_min_max_per_step(
